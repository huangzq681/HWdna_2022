...
def get_file(forc,ensem,start='19500101',end='20191231'):
    if forc != 'historical':
        if ensem in ['r2i1p1f1','r4i1p1f1']:
            file_name = var_name + '_day_MRI-ESM2-0_' + forc + '_' + ensem + '_gn_' + start + '-' + end + '_level' + str(sel_level) + '.nc'
        else:
            file_name = var_name + '_day_MRI-ESM2-0_' + forc + '_' + ensem + '_gn_' + start + '-' + '20141231' + '_level' + str(sel_level) + '.nc'
    else:
        file_name = var_name + '_day_MRI-ESM2-0_' + forc + '_' + ensem + '_gn_' + start + '-' + '20141231' + '_level' + str(sel_level) + '.nc'
    return file_name

def to_file(forc,ensem,content,start=time_start,end=time_end):
	file_name = var_name + '_day_MRI-ESM2-0_' + forc + '_' + ensem + '_gn_' + start + '-' + end + '_level' + str(sel_level) + '_' + content + '.nc'
	return file_name

for f in forcs:
    for e in ensems:
        to_file_path = to_file(f,e,'clim')
