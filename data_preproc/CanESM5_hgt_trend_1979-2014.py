...
def get_file(forc,ensem,start='19510101',end='20201231'):
    if forc != 'historical':
        file_name = var_name + '_day_CanESM5_' + forc + '_' + ensem + '_gn_' + start + '-' + end + '_level' + str(sel_level) + '.nc'
    else:
        file_name = var_name + '_day_CanESM5_' + forc + '_' + ensem + '_gn_' + start + '-' + '20141231' + '_level' + str(sel_level) + '.nc'
    return file_name

def to_file(forc,ensem,content,start=time_start,end=time_end):
	file_name = var_name + '_day_CanESM5_' + forc + '_' + ensem + '_gn_' + start + '-' + end + '_level' + str(sel_level) + '_' + content + '.nc'
	return file_name
