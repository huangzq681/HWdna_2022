...
ensems = ['r1i1p1f1','r2i1p1f1','r4i1p1f1','r5i1p1f1','r6i1p1f1','r7i1p1f1']
...
def get_file(forc,ensem,start='18500101',end='20201231'):
    if forc != 'historical':
        file_name = var_name + '_day_IPSL-CM6A-LR_' + forc + '_' + ensem + '_gr_' + start + '-' + end +'.nc'
        if forc == 'hist-nat' and ensem in ['r4i1p1f1','r5i1p1f1']:
            file_name = var_name + '_day_IPSL-CM6A-LR_' + forc + '_' + ensem + '_gr_' + '19270101' + '-' + end +'.nc'
    else:
        file_name = var_name + '_day_IPSL-CM6A-LR_' + forc + '_' + ensem + '_gr_' + start + '-' + '20141231' + '.nc'
    return file_name

def to_file(forc,ensem,content,start=time_start,end=time_end):
	file_name = var_name + '_day_IPSL-CM6A-LR_' + forc + '_' + ensem + '_gr_' + start + '-' + end + '_' + content + '.nc'
	return file_name
