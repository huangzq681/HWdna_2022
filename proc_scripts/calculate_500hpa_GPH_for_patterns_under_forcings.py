...
def to_ncfile(domain,pattern,forcing):
    to_dir = '/lustre07/scratch/xtan/hzq/HWdna/procData/' + 'hgt_patt'+ str(pattern + 1) +'_average_' + forcing + '_' + domain + '.nc'
    return to_dir

def calculate_patt_avg_gph(forcing,domain):
...
# for f in ['historical','hist-GHG','hist-aer','hist-nat','ssp585']:
    for d in ['EU','EAS','WNA']:
        calculate_patt_avg_gph(forcing=f,domain=d)
        print(f + '_' + d)
