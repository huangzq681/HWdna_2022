...
def cal_hot_extreme_occur(dataset_name,domain,forcing=None,run=None):
    ...
    if dataset_name in ['era5','jra55','ncep2']:
        ...
        season_cycle_quantile = season_cycle_quantile.squeeze(drop=True)
    else:
...
def save_result(domain):
    to_file_dir = '/home/xtan/scratch/hzq/HWdna/procData/'
    tmax_occur_reanalyses = pd.DataFrame(columns=['era5','jra55','ncep2'])

    path = to_file_dir + 'hot_extreme_occur_reanlyses_yearly_all_patterns_' + domain + '.csv'
    if os.path.exists(path) == True:
        pass
    else:
        for d in ['era5','jra55','ncep2']:
            tmax_occur = cal_hot_extreme_occur(dataset_name=d,domain=domain)
            tmax_occur = tmax_occur.to_series()
            print(tmax_occur)
            # tmax_occur.index = range(1979,2015)
            tmax_occur_reanalyses[d] = tmax_occur
            print(d)
        tmax_occur_reanalyses.to_csv(to_file_dir + 'hot_extreme_occur_reanlyses_yearly_all_patterns_' + domain + '.csv')
        ...
        if os.path.exists(path) == True:
            ...
            for d in dataset_src_run.keys():
                for r in dataset_src_run[d][f]:
                    tmax_occur = cal_hot_extreme_occur(dataset_name=d,domain=domain,forcing=f,run=r)
                    tmax_occur = tmax_occur.to_series()
                    print(tmax_occur)
                    # tmax_occur.index = range(1979,2015)
                    tmax_occur_cmip[d + '_' + r] = tmax_occur
                    print(f + '_' + d + '_' + r)
