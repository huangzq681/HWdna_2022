...
def to_ncfile(domain,forcing):
    to_dir = '/lustre07/scratch/xtan/hzq/HWdna/procData/' + 'hgt_average_' + forcing + '_' + domain + '.nc'
    return to_dir

def calculate_avg_gph(forcing,domain):
...
for f in ['historical']:
    for d in ['EU','EAS','WNA']:
        calculate_avg_gph(forcing=f,domain=d)
        print(f + '_' + d)
