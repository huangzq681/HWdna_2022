...
def sel_domain(dataset_name,domain,forcing=None,run=None):
    ...
    if forcing != 'ssp585' and forcing != 'piControl': 
        ...
        if dataset_name == 'HadGEM3-GC31-LL':
            ...
            time_start = '2064-01-01'
            time_end = '2099-12-31'
    elif forcing == 'piControl':
        time_range_pi = time_range_hgt[dataset_name]['piControl']
        time_start = time_range_pi[0][:4] + '-' + time_range_pi[0][4:6] + '-' + time_range_pi[0][6:]
        time_end = time_range_pi[1][:4] + '-' + time_range_pi[1][4:6] + '-' + time_range_pi[1][6:]
    else:
        pass
...
def save_result(domain):
    to_file_dir = '/home/xtan/scratch/hzq/HWdna/procData/'
    path = to_file_dir + 'GPH_piControl_yearly_all_patterns_' + domain + '.csv'
    ...
    for f in ['piControl']:
        ...
        for d in dataset_src_run.keys():
            for r in dataset_src_run[d][f]:
                ...
                print(hgt_occur)
                hgt_occur.index = range(len(hgt_occur))
